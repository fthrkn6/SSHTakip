...
class RootCauseAnalysis(db.Model):
    """Root Cause Analysis modeli - Arıza nedeni analizi"""
    __tablename__ = 'root_cause_analysis'
    
    id = db.Column(db.Integer, primary_key=True)
    tram_id = db.Column(db.String(50), nullable=False)
    failure_id = db.Column(db.Integer, db.ForeignKey('failures.id'))
    sistem = db.Column(db.String(100), nullable=False)  # Sistem adı
    alt_sistem = db.Column(db.String(100))  # Alt sistem
    root_cause = db.Column(db.Text, nullable=False)  # Temel neden
    immediate_cause = db.Column(db.Text)  # Acil neden
    contributing_factors = db.Column(db.Text)  # Katkıda bulunan faktörler (JSON list)
    corrective_actions = db.Column(db.Text)  # Düzeltici eylemler (JSON list)
    preventive_actions = db.Column(db.Text)  # Önleyici eylemler (JSON list)
    severity = db.Column(db.String(20), default='medium')  # low, medium, high, critical
    failure_mode = db.Column(db.String(100))  # Arıza modu
    analysis_date = db.Column(db.DateTime, default=datetime.utcnow)
    analyzed_by = db.Column(db.Integer, db.ForeignKey('users.id'))
    status = db.Column(db.String(20), default='draft')  # draft, submitted, approved, closed
    notes = db.Column(db.Text)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    failure = db.relationship('Failure', backref='root_causes')
    analyzer = db.relationship('User', backref='root_cause_analyses')
    
    def __repr__(self):
        return f'<RootCauseAnalysis {self.tram_id} - {self.sistem}>'


class AvailabilityReport(db.Model):
    """Availability rapor modeli - Haftalık, aylık, ylık raporlar"""
    __tablename__ = 'availability_reports'
    
    id = db.Column(db.Integer, primary_key=True)
    tram_id = db.Column(db.String(50), nullable=False)
    report_period = db.Column(db.String(50), nullable=False)  # daily, weekly, monthly, quarterly, biannual, yearly, total
    period_start = db.Column(db.DateTime, nullable=False)
    period_end = db.Column(db.DateTime, nullable=False)
    
    # Availability metrikleri
    available_hours = db.Column(db.Float, default=0)  # Servis saatleri
    unavailable_hours = db.Column(db.Float, default=0)  # Servis dışı saatleri
    total_hours = db.Column(db.Float, default=0)  # Toplam saatler
    availability_percentage = db.Column(db.Float, default=0)  # Availability yüzdesi
    
    # Breakdown
    maintenance_hours = db.Column(db.Float, default=0)  # Planlı bakım
    failure_hours = db.Column(db.Float, default=0)  # Arızadan dolayı kapalı
    operational_downtime = db.Column(db.Float, default=0)  # İşletme kaynaklı kapalı
    
    # Detail raporu
    sistema_gore_breakdown = db.Column(db.Text)  # JSON: sistem başına breakdown
    daily_breakdown = db.Column(db.Text)  # JSON: günlük breakdown
    
    generated_by = db.Column(db.Integer, db.ForeignKey('users.id'))
    generated_at = db.Column(db.DateTime, default=datetime.utcnow)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    
    user = db.relationship('User', backref='availability_reports')
    
    def __repr__(self):
        return f'<AvailabilityReport {self.tram_id} - {self.report_period}>'


class ServiceLog(db.Model):
    """Servis durumu log modeli - Tüm durum değişikliklerinin kaydı"""
    __tablename__ = 'service_logs'
    
    id = db.Column(db.Integer, primary_key=True)
    tram_id = db.Column(db.String(50), nullable=False, index=True)
    ...
    alt_sistem = db.Column(db.String(100))
    reason = db.Column(db.Text)  # Durum değişikliğinin nedeni
    duration_hours = db.Column(db.Float)  # Durağan kalış süresi
    created_by = db.Column(db.Integer, db.ForeignKey('users.id'))
    notes = db.Column(db.Text)
    ...
    def __repr__(self):
        return f'<ServiceLog {self.tram_id} - {self.log_date}: {self.new_status}>'


class AvailabilityMetrics(db.Model):
    """Availability Metrikleri - Araç kullanılabilirlik hesaplamaları"""
    ...
    # Temel Metrikleri
    total_hours = db.Column(db.Float, default=24)  # Toplam saatler
    operational_hours = db.Column(db.Float, default=0)  # Operasyon saatleri
    downtime_hours = db.Column(db.Float, default=0)  # Durağan kalış saatleri
    maintenance_hours = db.Column(db.Float, default=0)  # Bakım saatleri
    availability_percentage = db.Column(db.Float, default=0)  # %
    
    # Detaylı Downtime Kategorileri
    maintenance_downtime = db.Column(db.Float, default=0)
    unplanned_downtime = db.Column(db.Float, default=0)
    planned_downtime = db.Column(db.Float, default=0)
    other_downtime = db.Column(db.Float, default=0)
    
    # MTBF / MTTR
    mtbf_hours = db.Column(db.Float)  # Mean Time Between Failures
    mttr_hours = db.Column(db.Float)  # Mean Time To Repair
    mttm_hours = db.Column(db.Float)  # Mean Time To Maintenance
    ...
    report_period = db.Column(db.String(50))  # daily, weekly, monthly, quarterly, yearly
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    
    __table_args__ = (db.Index('ix_tram_date_period', 'tram_id', 'metric_date', 'report_period'),)
